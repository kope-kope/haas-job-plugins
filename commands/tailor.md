...
5. If Google Drive is configured in `${CLAUDE_PLUGIN_ROOT}/references/config.md`, also copy the master resume template and replace content
6. Write a gap report in the chat

Always read `${CLAUDE_PLUGIN_ROOT}/skills/resume-tailor/references/resume.md` first. If the file is missing or empty, tell the user to run `/setup` first.
